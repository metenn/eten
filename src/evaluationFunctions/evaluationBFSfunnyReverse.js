// @ts-nocheck
module.exports = function evaluationBFSfunnyReverse(board) {
    // right goal
    ...
    if (!vis[1][4]) return (board.ball[0] - 6) * (board.ball[0] - 6);

    // impossible to goal right
    if (!vis[11][4]) return -(board.ball[0] - 6) * (board.ball[0] - 6);
