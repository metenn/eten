...
function getEmbed(film, interaction) {
    ...
    if (film.playlist) {
        embed
            ...
            .setDescription(
                `Dodano playlistę do kolejki (${film.playlist.tracks.length} ${film.playlist.source == "youtube" ? "filmów" : "piosenek"} na playliście)`,
            )
            .setAuthor({
        ...
        embed
            ...
            .setDescription(
                `Dodano ${film.tracks[0].source == "youtube" ? "film" : "piosenkę"} do kolejki (${film.tracks[0].duration})`,
            )
            .setAuthor({
                name: interaction.user.username,
                // @ts-expect-error
    ...
    if (queue) {
        ...
        try {
            ...
            interaction.editReply(
                "Nie mogłem dołączyć do vc:( <@230917788699459584>",
            );
            return;
        }
