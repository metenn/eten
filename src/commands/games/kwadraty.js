...
export async function execute(interaction) {
    if (interaction.isButton()) {
        ...
        if (!uids[interaction.user.id]) return;
        ...
        if (ranking.kwadraty[uid1] === undefined)
            ranking.kwadraty[uid1] = {
                lost: 0,
                won: 0,
                rating: ranking.kwadraty[uid1].rating
                    ? ranking.kwadraty[uid1].rating
                    : 1500,
            };

        if (ranking.kwadraty[uid2] === undefined)
            ranking.kwadraty[uid2] = {
                lost: 0,
                won: 0,
                rating: ranking.kwadraty[uid2].rating
                    ? ranking.kwadraty[uid2].rating
                    : 1500,
            };
...
export async function onMessage(message) {
    ...
    if (!boards[id].move(message.content)) return;
    ...
    if (boards[id].win == -1) components = true;
...
async function remisManager(interaction, mainMessage) {
    ...
    if (boards[id].remis.includes(uid)) return;
...
async function acceptManager(interaction, mainMessage) {
    ...
    if (to != interaction.user.id) return;
    ...
    if (interaction.customId.startsWith("acceptNo")) {
        for (let i = 0; i < accepts.length; i++) {
            ...
            if (accept.to == to && accept.from == from) {
                ...
                /** @type {Message} */ (accept.message).edit({
                    content: msg,
                    components: [],
                });
                accepts.splice(i, 1);
        ...
        const newAccepts = [];
        for (const a of accepts) {
            if (a.from != from && a.to != to) newAccepts.push(a);
            /** @type {Message} */
            // @ts-expect-error
            else a.message.edit({ content: a.message.content, components: [] });
        }
...
async function sendBoard(
    ...
    else message = await /** @type {Message} */ (message).edit(messagePayload);
