...
export async function execute(interaction) {
    ...
    if (
        ...
        if (miasto != "warszawa") {
            interaction.editReply("Nie chce mi sie bardziej naprawiac");
            return;
        }
        title = "Warszawa";
        ...
        if (!imgResult.ok)
            throw new Error(`Unexpected response ${imgResult.statusText}`);
    } else {
        ...
        const resultText = await result.text();
        const imageRegex =
            /src="(https:\/\/www\.meteo\.pl\/um\/metco\/mgram_pict\.php\?ntype=0u&fdate=[0-9]+&row=406&col=250&lang=pl)"/g;
        // @ts-expect-error
