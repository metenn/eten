...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption(
        new SlashCommandStringOption()
            ...
            .addChoices({
                name: "Drużynowe Pilkarzyki",
                value: "teampilkarzyki",
            })
            .addChoices({ name: "Najdluższy ruch", value: "najdluzszyruch" })
            .addChoices({
                name: "Najdluższa gra w drużynowych piłkarzykach",
                value: "najdluzszagrateampilkarzyki",
            })
            .addChoices({
                name: "Najdluższa gra w piłkarzykach",
                value: "najdluzszagrapilkarzyki",
            })
            ...
            .addChoices({
                name: "Prawdopodobieństwo na cyferki",
                value: "dubspercent",
            }),
    )
    .addUserOption(
...
export async function execute(interaction) {
    ...
    let desc = "";
    if (
        type == "najdluzszagrapilkarzyki" ||
        type == "najdluzszagrateampilkarzyki" ||
        type == "najdluzszyruch" ||
        type == "sumaruchow" ||
        type == "jajco" ||
        type == "bets"
        ...
        const dubRankinkg =
            /** @type {import("../../../types.js").IRanking} */ (fullRanking)
                .dubs;
        if (mentionedUser == null) {
            ...
            for (const [userId, dubRecord] of Object.entries(dubRankinkg)) {
                ...
                for (const dubTier of Object.keys(dubRecord)) {
                    if (Number(dubTier) > topDub) topDub = Number(dubTier);
                }
            ...
            userTopDubs.sort(function (a, b) {
                if (a.topDub === b.topDub)
                    return (
                        dubRankinkg[b.user][b.topDub] -
                        dubRankinkg[a.user][a.topDub]
                    );
            ...
            for (let i = 0; i < Math.min(15, userTopDubs.length); i++) {
                ...
                desc = desc.concat(
                    `<@${userTopDub.user}>: ${repeatingDigitsText[userTopDub.topDub]} x${dubRankinkg[userTopDub.user][userTopDub.topDub]}\n`,
                );
            }
        } else {
            ...
            if (!(mentionedUser.id in dubRankinkg)) {
                desc = desc.concat(
                    "Nie doświadczył żadnych powtarzających się numerków",
                );
            } else {
                // Sorted by top dub
                /** @type {Array<{ dubTier: number, count: number; }>} */
                const userDubsArr = [];
                for (const [dubTier, count] of Object.entries(
                    dubRankinkg[mentionedUser.id],
                )) {
                    userDubsArr.push({
                        dubTier: Number(dubTier),
                        count: count,
                    });
                }
                ...
                for (const entry of userDubsArr) {
                    if (entry.dubTier == 1) break;
                    desc = desc.concat(
                        `${repeatingDigitsText[entry.dubTier]} x${entry.count}\n`,
                    );
                }
                desc = desc.concat(
                    `Bez powtarzania: ${dubRankinkg[mentionedUser.id][1]}`,
                );
            }
        }
        ...
        let total = 0;
        for (let i = 1; i <= max; i++) total += arr[i];
        // Append desc
        for (let i = 1; i <= max; i++) {
            desc = desc.concat(
                `${repeatingDigitsText[i]}: ${((arr[i] / total) * 100).toFixed(3)}%\n`,
            );
        }
    } else {
        for (const [key, value] of Object.entries(ranking)) {
    ...
    if (type == "pilkarzyki") title = "Ranking piłkarzyków";
    else if (type == "kwadraty") title = "Ranking kwadratów";
    ...
    else if (type == "najdluzszyruch") title = "Ranking najdłuższych ruchów";
    else if (type == "sumaruchow") title = "Ranking ilości ruchów";
    else if (type == "jajco") title = "Ranking przegranych w jajco";
    else if (type == "bets")
        title = "Ranking wygranych zakładów o godzine postowania ogłoszeń";
    else if (type === "dubs") title = "Ranking poświadczonych numerków";
    else if (type === "dubspercent")
