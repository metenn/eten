...
async function randomSoundOnVoice() {
    ...
    client.guilds.cache.forEach((guild) => {
        for (const channel of guild.channels.cache
            ...
            .values())
            channels.push(
                /** @type {import("discord.js").VoiceChannel} */ (channel),
            );
    });
    ...
    for (const channel of channels.values()) {
        if (
            ...
            Math.random() >= config.randomSoundeffectChance
        )
            continue;
        isThereAnyone = true;
        ...
        if (isAlreadyOnVC)
            // TODO ~~lepiej to~~ nwm o co mi tu chodzilo zeby lepiej zrobic
            continue;
        ...
        const resource = createAudioResource(
            "./soundeffects/" + files[Math.floor(Math.random() * files.length)],
        );
...
export default async function () {
    if (config.playRandomSoundeffects) randomSoundOnVoice();
}
