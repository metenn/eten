...
async function coCountdown(client) {
	try {
		...
		if (coUsers.count >= 0) {
			// @ts-expect-error
			/** @type {import("discord.js").TextChannel} */(client.channels.cache.get(coChannel)).send(String(coUsers.count)).then(message => {
		...
		// @ts-expect-error
		/** @type {import("discord.js").TextChannel} */(client.channels.cache.get(coChannel)).send(msg);
		coChannel = undefined;
		coUsers = undefined;
	}
	catch (error) {
