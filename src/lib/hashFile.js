...
export async function hashFile(url, message) {
    ...
    await streamPipeline(
        // @ts-expect-error
        imgResult.body,
        fs.createWriteStream("./tmp/tmp." + extension),
    );
    ...
    if (hexHash in hashes[message.guildId]) {
        ...
        let repost;
        try {
            repost = await /** @type {import("discord.js").TextChannel} */ (
                // @ts-expect-error
                message.client.guilds.cache
                    .get(splits[0])
                    .channels.cache.get(splits[1])
            ).messages.fetch(splits[2]);
        } catch (error) {
            repost = undefined;
        }
...
export async function hashFileFromMessageContent(message) {
    const regex =
        /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*).(webm|mp4|mov|avi|flv|mkv|wmv|m4v|png|jpg|gif|jpeg|webp|svg|ovg|ogg)\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/gi;
    let match;
    while ((match = regex.exec(message.content))) {
        const url = message.content.slice(
            regex.lastIndex - match[0].length,
            regex.lastIndex,
        );
        if (url.search("tenor.com") == -1) {
            // TODO fuch me
            try {
                // ?? to działało??
