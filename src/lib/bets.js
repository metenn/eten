...
export async function addTime(time) {
    const predictions = JSON.parse(
        fs.readFileSync("./data/predictions.json", "utf-8"),
    );
...
export async function check(date) {
    // eneabled?
    if (!config.bets.eneabled) return;
    ...
    for (const [user, time] of Object.entries(bets)) {
        ...
        let niceDiff = "";
        if (!cheated) {
            if (time.time > now) niceDiff = "za pózno o ";
            else niceDiff = "za wcześnie o ";
        }

        if (hours > 0) niceDiff += `${hoursS}:${minsS} godzin`;
        else if (mins > 0) niceDiff += `${minsS} minut`;
        else if (sec > 0) niceDiff += `${secS} sekund`;
        else if (mili > 0) niceDiff += `${miliS} milisekund`;

        if (cheated) niceDiff += " temu";
    ...
    users.sort((a, b) => {
        if (a.diff == b.diff) return a.timeAdded - b.timeAdded;
    ...
    if (users.length != 0) {
        ...
        embed = new EmbedBuilder()
            ...
            .setFooter({
                text:
                    "Ogłoszenie dodane o " + date.toTimeString().split(" ")[0],
            });

        /** @type {import("discord.js").TextChannel} */ (
            // @ts-expect-error
            client.guilds.cache
                .get(config.bets.guild)
                .channels.cache.get(config.bets.channel)
        ).send({ embeds: [embed] });
        ...
        if (!(users[0].user in ranking.bets)) ranking.bets[users[0].user] = 0;
