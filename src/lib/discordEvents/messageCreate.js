...
export default async function (message) {
    ...
    if (repeatingDigits >= 3) {
        if (repeatingDigits >= 6) {
            message
                .react("<:checkem:966379892474249246>")
                .catch((error) => console.log(error));
            message.reply({
                content: `@everyone WITNESSED (ID: ${message.id.substring(0, message.id.length - repeatingDigits)}**${message.id.substring(message.id.length - repeatingDigits)}**)`,
                files: [
                    "https://www.vogue.pl/uploads/repository/nina_p/ap.jpg",
                ],
            ...
            message
                ...
                .then(() => {
                    message
                        .react(repeatingDigitsText[repeatingDigits])
                        .catch((error) => console.log(error));
                })
                .catch((error) => console.log(error));
        }
    ...
    if (
        ...
        message.content.toLowerCase() == "czym do chuja są te numerki?"
    ) {
...
https://cdn.discordapp.com/attachments/856926964094337047/968536776484487218/unknown.png`);
    ...
    if (message.channel.id === "813703962838564865") {
        try {
            await message
                .react("<:among_us:754362953104359747>")
                .catch((error) => console.log(error));
        } catch (error) {
    ...
    if (message.content.length == 4) onMessage(message);
    ...
    if (message.attachments.size > 0) {
        for (const [id, attachment] of message.attachments) {
            ...
            if (
                attachment.name.toLowerCase().match("reddit") ||
                attachment.name.toLowerCase().match("RDT") ||
                redditFilenameRegex.test(attachment.name)
            )
                reddit = true;
        }
    }
    ...
    if (
        /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*).(webm|mp4|mov|avi|flv|mkv|wmv|m4v|png|jpg|gif|jpeg|webp|svg|ovg|ogg)\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/g.test(
            message.content,
        )
    ) {
    ...
    if (reddit && false) {
        message.reply({
            content:
                "https://cdn.discordapp.com/attachments/788126323180044339/980463018800459866/Ew_You_Must_Be_From_Reddit_pl7RIIHK9EE.mp4",
        });
    }
