...
import { checkReactionAdd } from "../staszic/checkReaction.js";
...
export default async function (reaction, reactedUser) {
    ...
    const reactionMessages =
        /** @type {import("../../../types.js").IReactionMessages} */ (
            JSON.parse(fs.readFileSync("./data/reactionMessages.json", "utf-8"))
        );
    if (!(reaction.message.id in reactionMessages)) return;

    const message = await /** @type {TextChannel} */ (
        client.channels.cache.get(reaction.message.channel.id)
