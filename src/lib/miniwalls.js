...
async function getCount() {
    // @ts-ignore
    const result = await fetch("https://api.hypixel.net/v2/counts", {
...
async function check() {
    ...
    // @ts-expect-error
    client.user.setActivity(`Mini Walls (${count} graczy)`, {
        // @ts-expect-error
        type: "PLAYING",
    });
...
export default function startCron() {
    const job = new cron.CronJob("0,10,20,30,40,50 * * * *", check);
    check();
    job.start();
}
