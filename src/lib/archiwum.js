...
export default async function (message) {
    if (
        config.archiwum.eneabled &&
        message.channel.id == config.archiwum.channel &&
        ...
        await streamPipeline(
            // @ts-expect-error
            res.body,
            fs.createWriteStream("tmp/tmparchive" + ext),
        );

        const form = new formData();
        const stats = fs.statSync("tmp/tmparchive" + ext);
        const size = stats.size;
        form.append("image", fs.createReadStream("tmp/tmparchive" + ext), {
            knownLength: size,
        });
