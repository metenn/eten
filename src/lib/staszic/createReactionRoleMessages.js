...
export async function sendSzczesliwyNumerekMessages(channelID) {
    ...
    for (let i = 0; i < 4; i++) {
        let desc =
            "Kliknij odpowiednią reakcję, żeby wybrać swój numerek w dzienniku\n\n";
        for (let j = 1; j <= 10; j++) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(
                /** @type {import("discord.js").ColorResolvable} */ (colors[i]),
            )
            .setDescription(desc);
...
export async function sendMessages(channelID) {
    ...
    for (const max of klasy) {
        let desc =
            "Kliknij odpowiednią reakcję, żeby dostać rolę swojej klasy\n\n";
        for (
            let letter = "A";
            letter <= max;
...
export async function editMessage(channelID, msgID) {
    ...
    for (
        ...
        if (letter != "H") desc += `${letters[letter]} - 1${letter}\n`;
    }
...
export async function upgradeRoles(guildID, graduateRoleId) {
    ...
    const graduateRole = guild.roles.cache.find(
        (role) => role.id == graduateRoleId,
    );
    if (graduateRole == undefined) throw new Error("Graduate role not found");
