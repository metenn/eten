...
export default class Board extends BaseBoard {
	 ...
	 */
	constructor(spacing, offsetX, offsetY, thickness, uids, usernames, id = 0, withBot = false) {
		super(spacing, offsetX, offsetY, thickness, uids, usernames, id);
		this.withBot = withBot;

		const settings = /** @type {import("../../../types.d.ts").IUserSettings} */ (JSON.parse(fs.readFileSync("./data/userSettings.json", "utf8")));
	...
	move(index) {
		...
		if (!this.withBot) {
			const ranking = /** @type {import("../../../types.d.ts").IRanking} */ (JSON.parse(fs.readFileSync("./data/ranking.json", "utf8")));
