...
export async function create() {
	...
	for (const create of toCreate) {
		const category = /** @type {import("discord.js").CategoryChannel} */ (await client.channels.fetch(create.categoryId));
		...
		for (let i = 1; i <= przedmiot.ammount; i++) {
			...
			if (i % 10 == 0) {
				const embed = new EmbedBuilder()
					...
					.setDescription(desc)
					.setColor(/** @type {import("discord.js").ColorResolvable} */(colors[colorI]));
				colorI++;
