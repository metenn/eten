...
export default class ExtBoard {
    constructor(board, size_hor, size_ver, evalFunc) {
        ...
        for (let x = 0; x < size_ver; ++x) {
            ...
            for (let y = 0; y < size_hor; ++y) this.graph[x][y] = new Array(8);
        }
        for (let x = 1; x < size_ver - 1; ++x) {
            for (let y = 1; y < size_hor - 1; ++y) {
                if (x == 1 && !this.isGoal([x, y])) continue;
                if (x == 11 && !this.isGoal([x, y])) continue;
                for (let i = 0; i < 8; ++i) this.graph[x][y][i] = true;
        ...
        if (evalFunc === null) evalFunc = evalFunctionDefault;
    ...
    load(filename) {
        const data = fs
            .readFileSync(SAVE_PATH + filename, { encoding: "utf8" })
            .split("#");
        ...
        if (b > 1) {
            for (let i = 0; i < a; ++i) {
                ...
                if (c > 1) {
                    for (let j = 0; j < b; ++j) {
                        res[i][j] = Array(c);
                        for (let k = 0; k < c; ++k) res[i][j][k] = defaultVal;
                    }
                } else {
                    for (let j = 0; j < b; ++j) res[i][j] = defaultVal;
                }
            }
        } else {
            for (let i = 0; i < a; ++i) res[i] = defaultVal;
        }
        return res;
    }
    ...
    possibleDirections(point) {
        const res = [];
        for (let i = 0; i < 8; ++i) {
            if (this.graph[point[0]][point[1]][i] === false) res.push(i);
        }
        return res;
    }
    ...
    makeMove(move) {
        for (const dir of move) {
            if (this.graph[this.ball[0]][this.ball[1]][dir])
                console.log(
                    "OH NO: there is already edge %o --- %o",
                    this.ball,
                    this.moved(this.ball, dir),
                );
    ...
    unmakeMove(move) {
        for (const dir of move.reverse()) {
            if (!this.graph[this.ball[0]][this.ball[1]][7 - dir])
                console.log(
                    "OH NO: there was no edge %o --- %o",
                    this.ball,
                    this.moved(this.ball, 7 - dir),
                );
    ...
    single(s, t, canGoFurther, player) {
        ++this.xd;
        if (s[0] == t[0] && s[1] == t[1]) return [[this.evalFunc(this), []]];

        if (s[0] == 1 || s[0] == 11) {
            if (t[0] == 1 || t[0] == 11) return [[this.evalFunc(this), []]];
            return null;
        }
            ...
            for (const i of this.possibleDirections(s)) {
                ...
                if (path !== null && path.length > 0) {
                    ...
                    for (const tmp_path of path) {
                        if (tmp_path === null) continue;

                        queue.queue([tmp_path[0], [i].concat(tmp_path[1])]);
                        if (queue.length > 50) queue.dequeue();
                    }
                }
            }
        }
        ...
        const res = [];
        while (queue.length) res.push(queue.dequeue());
        return res;
    }
    ...
    BFS(start) {
        ...
        while (queue.length) {
            ...
            for (const i of this.possibleDirections(v)) {
                ...
                if (this.isGoal(w)) canGoFurther = false;

                if (!vis[w[0]][w[1]]) {
                    vis[w[0]][w[1]] = true;
                    points.push(w);
                    if (canGoFurther) queue.push(w);
                }
            }
        }
    ...
    orderMoves(moves, player) {
        ...
        for (const point of moves) {
            if (point[0] == (player === 0 ? 11 : 1)) forcing.push(point);
            else res.push(point);
        }
    ...
    search(depth, player, alpha, beta) {
        ...
        if (depth == 0 || Math.abs(evaluation) == 1000) return [evaluation, []];
        ...
        for (const [x, y] of this.orderMoves(this.generateMoves(), player)) {
            ...
            if (!end && !this.isGoal([x, y])) continue;
            ...
            if (this.xd > 1000000) {
                console.log(
                    "FindPath %d (player %d) from [%d, %d] to [%d, %d]",
                    this.xd,
                    player,
                    this.ball[0],
                    this.ball[1],
            ...
            for (const [_, path] of paths.reverse()) {
                if (path !== null && path.length > 0) {
                    ...
                    if (player == 0) {
                        ...
                        if (val[0] > best[0]) best = [val[0], path];
